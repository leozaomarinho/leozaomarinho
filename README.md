<div align="center">
  <h1>👋 Prazer, eu sou Leonardo Marinho!</h1>
  <p><strong>Analista de Segurança da Informação Pleno III | AppSec & DevSecOps Specialist</strong></p>
  
  <p>
    <a href="https://www.linkedin.com/in/leonardo-l-marinho" target="_blank">
      <img src="https://img.shields.io/badge/-LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
    </a>
    <a href="mailto:leowalkg6@gmail.com">
      <img src="https://img.shields.io/badge/-Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email">
    </a>
  </p>
</div>

---

### 🛡️ Sobre Mim
Profissional de Segurança da Informação com **mais de 4 anos de experiência** focados em **AppSec** e **DevSecOps**. Atuo na implementação e evolução da segurança em pipelines CI/CD de grande porte, integrando ferramentas de **SAST, SCA e Secret Scanning**, automação de processos em Python/Bash e apoio à conformidade com **PCI DSS**, **ISO 27001**, **NIST**, **CIS Controls** e **OWASP (Top 10, ASVS, SAMM)**.

---

### 🛠️ Principais Competências & Tech Stack

- **AppSec & SSDLC:** Fortify, Dependency-Track, Conviso, Cloudflare WAF, BitSight, DAST, Pentest
- **DevSecOps & CI/CD:** GitHub Actions, Azure DevOps, GitLab CI (Integração SAST/SCA/Secret Scanning)
- **Automação & Linguagens:** Python, Bash, Java, .NET, C#
- **Frameworks & Compliance:** OWASP (Top 10 / ASVS / SAMM), NIST, CIS Controls, MITRE ATT&CK, PCI DSS, ISO 27001

---

### 💼 Experiência Relevante
- 🚀 **Analista de Segurança da Informação Pleno III** @ *Evernow Consultoria* *(2026 - Atual)*
- 🏦 **Analista de Segurança da Informação** @ *Banco Inter* *(2023 - 2026)*
- 🎓 **Analista de Segurança da Informação** @ *Ânima Educação* *(2021 - 2023)*

---

### 🎓 Formação Acadêmica
- **Pós-graduação Lato Sensu em Segurança da Informação** — *Centro Universitário Una* *(2024 - 2025)*
- **CST em Análise e Desenvolvimento de Sistemas** — *Centro Universitário Una* *(2020 - 2022)*

---

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=leozaomarinho&show_icons=true&theme=tokyonight&hide_border=true" alt="Estatísticas GitHub" height="160">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=leozaomarinho&layout=compact&theme=tokyonight&hide_border=true" alt="Linguagens mais usadas" height="160">
</div>

